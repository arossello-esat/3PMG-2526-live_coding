...
struct Input {
    Input(Window& w);
    Input(Window& w,Input& other) : w_{w} , keys{std::move(other.keys)}{
...
struct Engine {
    std::optional<Engine> create() {
        auto maybe_window = Window::create();
        if(maybe_window) {
    ...
    Engine(Window w_) : w{std::move(w_)}, i{w} {}
    Engine(Engine& other) : w{std::move(other.w)}, i{w,std::move(other.i)} {}
};
